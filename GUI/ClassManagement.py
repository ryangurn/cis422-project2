...
class ClassManagement(tk.Tk):
    def __init__(self, master, studentName, db_file):
        ...
        self.courseSubjects = Listbox(self.window, selectmode="browse", bg=self._yellow, fg= self._darkGrey, selectbackground=self._green, font = subfont)
        ...
        self.offeredCourses = Listbox(self.window, selectmode = "browse", bg = self._lightGrey, fg= self._darkGrey, selectbackground=self._yellow, font = courseFont)
    ...
    def subjectClick(self, event):
        ...
        if not w.curselection() == ():
            ...
            cm = ClassModel.ClassModel(self.db)
            items = cm.find_by('subject', value)
